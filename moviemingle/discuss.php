...
<body>
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $queryGetName = "SELECT * FROM users WHERE user_id = '{$row['user_id']}'";
        $sql = mysqli_query($connectToSQL, $queryGetName);
        $data = mysqli_fetch_assoc($sql);
        echo "<h2 class='h2discuss'>" . $row["topic"] . "</h2>";
        echo "<div style=' margin-left:25px; display: flex; align-items:center;'><img style='width:50px; height:50px; border-radius:50%;' src='imgSignUp/{$data['profile_picture']}' alt=''><h5 class='h5discuss'>" ." by " . "{$data['username']}</h5></div>";
